...
def quiz():
    content = """
    한글 문법에 대한 퀴즈를 한 문제 내줘.
    사용자가 올바른 정답을 제시했다면 O, 틀렸다면 X를 답변으로 출력해.

    ###퀴즈 예시
    "영희는 학교에 갔습니다." 문장에서 "학교에"는 어떤 문법적 역할을 하나요?
    a) 주어 b) 목적어 c) 부사어 d) 보어

    "철수가 사과를 먹었다." 문장에서 "사과를"은 어떤 문법적 역할을 하나요?
    a) 주어 b) 목적어 c) 부사어 d) 보어

    다음 중 맞춤법이 올바른 문장은?
    a) 나는 내일 학교에 간다. b) 나는 내일 학교애 간다.

    "먹다"의 높임말은 무엇인가요?
    a) 드시다 b) 먹으시다 c) 드심다 d) 먹으심다

    다음 중 바른 문장은?
    a) 그 친구가 집에 갔데. b) 그 친구가 집에 갔대.
    """
def start_story():
    content = """출력 조건:
...
- 스토리 진행에 도움이 되는 '회복 키트'라는 아이템이 있다.
- 만약 '체력'이나 '멘탈'이 0이 됐을 때 '회복 키트' 아이템을 보유중이라면 '체력'과 '멘탈'이 1씩 증가하고 주인공은 죽지 않고 스토리를 이어갈 수 있다.
- '회복 키트'를 사용해 주인공이 부활한 경우 '회복 키트' 아이템은 사라진다.
